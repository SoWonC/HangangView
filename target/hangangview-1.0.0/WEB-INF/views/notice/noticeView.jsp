    ...
    <script>
        function confirmDelete() {
            if(confirm("해당 게시글을 삭제하시겠습니까?")) {
                window.location.href = '/project_1108/notice/delete?id=${notice.id}';
                alert("삭제가 완료되었습니다.");
            }
        }
        
        function goBack() {
            window.location.href = '/project_1108/notices?searchType=${param.searchType}&page=${param.page}&size=${param.size}&searchTerm=${param.searchTerm}';
        }
    </script>
</head>
<body>
...
<div class="notices_list">
    ...
    <p>등록일자 : <fmt:formatDate value="${notice.registerDate}" pattern="yyyy-MM-dd"/></p>
    <p>담당부서 : ${notice.department}</p>
    <p>담당자 : ${notice.personInCharge}</p>
    <p>전화번호 : ${notice.phoneNumber}</p>
    ...
    <button style="width: auto;" onclick="location.href='/project_1108/notices'">목록으로 돌아가기</button><br><br>

    <c:set var="sessionName" value="${sessionScope.authInfo != null ? sessionScope.authInfo.name : null}" />
    <c:if test="${sessionName eq 'admin'}">
        <button onclick="location.href='/project_1108/notice/modify?id=${notice.id}'">수정하기</button><br><br>
