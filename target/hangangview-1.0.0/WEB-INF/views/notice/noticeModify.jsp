...
<script>
    function deleteFile(fileId, noticeId) {
        ...
        xhr.open('GET', '/project_1108/notice/deleteFile?fileId=' + fileId + '&noticeId=' + noticeId, true);
    ...
    function submitForm() {
        if (confirm("해당 게시글을 수정하시겠습니까?")) {
            ...
            var xhr = new XMLHttpRequest();
            xhr.open("POST", "/project_1108/notice/modify", true);
            xhr.onreadystatechange = function() {
                if (xhr.readyState == 4 && xhr.status == 200) {
                    alert("수정이 완료되었습니다");
                    window.location.href = '/project_1108/notices';
                }
            };
            xhr.send(formData);
        }
...
<div class="notices_list">
    ...
    <form id="editForm" onsubmit="return submitForm()" method="post" enctype="multipart/form-data">
        ...
        <input type="text" id="department" name="department" value="${notice.department}" required/>
        <br>
        
        <label for="personInCharge">담당자:</label>
        <input type="text" id="personInCharge" name="personInCharge" value="${notice.personInCharge}" required readonly="true"/>
        <br>
        
        <label for="phoneNumber">전화번호:</label>
        <input type="text" id="phoneNumber" name="phoneNumber" value="${notice.phoneNumber}" required/>
        <br>
        
        <label for="content">내용:</label>
        <textarea id="content" name="content" rows="5" required>${notice.content}</textarea>
        <br>
        ...
        <ul>
            <c:forEach items="${attachedFiles}" var="file" varStatus="status">
                <li id="file-${file.fileId}">
                    ...
                    <a href="javascript:deleteFile(${file.fileId}, ${notice.id})">[삭제]</a>
                </li>
            </c:forEach>
        </ul>
    ...
    <button onclick="location.href='/project_1108/notice?id=${notice.id}'">뒤로가기</button>
	</div>
</body>
</html>