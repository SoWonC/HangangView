...
<script>
    function deleteFile(fileId, waterViewId) {
        var xhr = new XMLHttpRequest();
        xhr.open('GET', '/project_1108/waterView/deleteFile?fileId=' + fileId + '&waterViewId=' + waterViewId, true);
    ...
    function submitForm() {
        if (confirm("해당 게시글을 수정하시겠습니까?")) {
            ...
            var xhr = new XMLHttpRequest();
            xhr.open("POST", "/project_1108/waterView/modify", true);
            xhr.onreadystatechange = function() {
                if (xhr.readyState == 4 && xhr.status == 200) {
                    alert("수정이 완료되었습니다");
                    window.location.href = '/project_1108/waterViews';
                }
            };
            xhr.send(formData);
        }
...
<div class="notices_list">
    ...
    <form id="editForm" onsubmit="return submitForm()" method="post" enctype="multipart/form-data">
        <input type="hidden" name="id" value="${waterView.id}"/>
        
        <label for="title">제목:</label>
        <input type="text" id="title" name="title" value="${waterView.title}" required/>
        <br>
        
        <label for="department">담당부서:</label>
        <input type="text" id="department" name="department" value="${waterView.department}" required/>
        <br>
        
        <label for="personInCharge">담당자:</label>
        <input type="text" id="personInCharge" name="personInCharge" value="${waterView.personInCharge}" required readonly="true"/>
        <br>
        
        <label for="phoneNumber">전화번호:</label>
        <input type="text" id="phoneNumber" name="phoneNumber" value="${waterView.phoneNumber}" required/>
        <br>
        
        <label for="content">내용:</label>
        <textarea id="content" name="content" rows="5" required>${waterView.content}</textarea>
        <br>
        ...
        <ul>
            <c:forEach items="${attachedFiles}" var="file" varStatus="status">
                <li id="file-${file.fileId}">
                    ...
                    <a href="javascript:deleteFile(${file.fileId}, ${waterView.id})">[삭제]</a>
                </li>
            </c:forEach>
        </ul>
    ...
    <button onclick="location.href='/project_1108/waterView?id=${waterView.id}'">뒤로가기</button>
</div>
</body>
</html>
