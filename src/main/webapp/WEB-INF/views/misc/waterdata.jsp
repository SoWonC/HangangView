...
${wg.addr}
${wg.etcaddr}
<form action="waterdata" method="get">
    <select name="wlobscd" >
        <c:forEach var="dto" items="${wgs}">
            <option value="${dto.wlobscd}" selected="${dto.wlobscd == wlobscd}">${dto.etcaddr}</option>
         </c:forEach>
    </select>
    <input type="submit" value="보기">
</form>

<form action="waterdata" method="get" onsubmit="return validateForm()">
    <label>시작 날짜 및 시간 선택:</label>
    <input type="datetime-local" id="startDate" name="startDate" required>
    <label>끝 날짜 및 시간 선택:</label>
    <input type="datetime-local" id="endDate" name="endDate" required>

    <input type="hidden" name="wlobscd" value="${wlobscd}">
...
var dates = dataList.map(function(item) {
  ...
  var waterLevels = dataList.map(function(item) {
      return item.wl;
  });
  var flowRates = dataList.map(function(item) {
      return item.fw;
  });
  var attwl = dataList.map(function(item) {
      return item.attwl;
  });
  var wrnwl = dataList.map(function(item) {
      return item.wrnwl;
  });
  var almwl = dataList.map(function(item) {
      return item.almwl;
  });
  var srswl = dataList.map(function(item) {
      return item.srswl;
  });
